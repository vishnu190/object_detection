...
class facedetect():
    def get_frame():
        net = cv2.dnn.readNetFromDarknet("static_files/yolov4.cfg", "static_files/yolov4.weights")
        classes = []

        with open("static_files/coco.names", "r") as f:
        ...
        while True:
            ...
            cv2.imshow("Image", cv2.resize(frame, (900, 750)))
            key = cv2.waitKey(1)
            if key == 27:
